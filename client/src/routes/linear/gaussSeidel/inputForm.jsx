...
function ToggleForm({ dataGraph, setDataGraph, matrixSize, setMatrixSize, matrixA, setMatrixA, matrixB, setMatrixB, error }) {  
  const random = (...args) => {
    axios.get(
  ...
  const onClickCalculate = () => {
    const result = calculate(matrixSize, matrixA, matrixB, error);
    if (result) {
      setDataGraph(result);
    }
  };
  ...
  return (
    <>
      <div className="w-[26rem] mx-auto">
        ...
        <div className='mt-5'>
          <label htmlFor="error" className="mr-auto block mb-2 text-sm font-medium text-gray-90">
            Error
          </label>
          <input
            type="text"
